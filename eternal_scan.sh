...
ALL=`nmap -A --script=smb-vuln* -p $port $ip | tr " " "\n" | egrep -o -e "[0123456789]{1,3}\.[0123456789]{1,3}\.[0123456789]{1,3}\.[0123456789]{1,3}$" -e "CVE-2017-014[345678]" -e ^"[0123456789]{1,5}/"`
...
then
	...
	for ARG in $ALL
	do
		TEMP=`echo $ARG | egrep "[0123456789]{1,3}\.[0123456789]{1,3}\.[0123456789]{1,3}\.[0123456789]{1,3}"`
		if [[ $? -eq 0 ]]				# IP
		then
			IP=$TEMP
			if [[ $IP != $CURRENT_IP ]]
			...
			TEMP=`echo $ARG | egrep ^"[0123456789]{1,5}/"`
			if [[ $? -eq 0 ]]			# PORT
			then
				PORT=`echo $TEMP | tr '/' ' '`
				...
				TEMP=`echo $ARG | egrep "CVE-2017-014[345678]"`
				if [[ $? -eq 0 ]]		# CVE
				then
					CVE=$TEMP
